...
export default async function ProfilePage() {
  ...
  if (!identity) {
    redirect(getAuthChoicePath("/profile"));
  }

  const profile = await ensureProfileForUserId(identity.userId, identity);
  const firstName = identity.firstName ?? profile.displayName ?? "Not provided";
  const lastName =
    identity.lastName ?? (identity.authMode === "google" ? "Not provided" : "");
  const email =
    profile.email ?? identity.email ?? (identity.authMode === "google" ? "Not provided" : "");
