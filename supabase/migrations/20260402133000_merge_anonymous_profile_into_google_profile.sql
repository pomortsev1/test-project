create function public.merge_packing_profiles(
...
begin
  ...
  create temporary table if not exists profile_merge_category_map (
    source_id uuid primary key,
    target_id uuid not null,
    merged boolean not null
  ) on commit drop;

  delete from profile_merge_category_map;
...
grant execute on function public.merge_packing_profiles(uuid, uuid) to anon;
grant execute on function public.merge_packing_profiles(uuid, uuid) to authenticated;
grant execute on function public.merge_packing_profiles(uuid, uuid) to service_role;
